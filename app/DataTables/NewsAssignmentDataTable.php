...
class NewsAssignmentDataTable extends DataTable
{
    /**
    ...
    public function dataTable(QueryBuilder $query): EloquentDataTable
    {
        return (new EloquentDataTable($query))
            ...
            ->addColumn('action', function ($company) {
                return view('admin.pages.newsAssignment.partials.actions', compact('company'))->render();
            })
            ->addColumn('brand_name', function ($company) {
                ...
                $newsCountBadge = '<br><span class="badge badge-light-primary mt-1">News: ' . $company->news_count . '</span>';
                return $brandName . $sectorBadge . $newsCountBadge;
            })
    ...
    {
        return $model->select('id', 'uuid', 'logo', 'brand_name', 'company_name', 'list_order', 'category', 'sector_id', 'is_trending', 'share_price')
            ->with('sector:id,name') 
            ->withCount('news')
            ->where('is_deleted', '0');
    }
    ...
    {
        return $this->builder()
            ->setTableId('news_assignment_table')
    ...
    {
        return 'NewsAssignment_' . date('YmdHis');
    }
}
