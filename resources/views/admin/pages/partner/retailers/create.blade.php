<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('retailer.create') }}
    @endsection

    <div class="d-flex flex-column flex-lg-row">
        <div class="w-100 flex-lg-row-auto w-lg-1200px mb-7 me-7 me-lg-10">
            <form class="form" method="POST" action="{{ route('admin.partner.retailers.store') }}"
                enctype="multipart/form-data">
                @csrf
                <input type="hidden" name="partner_type" value="{{ App\Enums\PartnerTypeEnum::retailer }}">
                    ...
                    <div class="card-body pt-0">
                        ...
                        <div class="d-flex flex-wrap gap-10 mb-5">
                            <div class="fv-row fv-plugins-icon-container col-12 col-md-6 col-lg-4">
                                <label class="form-label">Select Parent(optional)</label>
                                <select class="form-select" name="parent_partner_id" aria-label="Select Parent Type">
                                    <option value="">-- Select Parent --</option>
                                    @foreach ($partners as $partner)
                                        <option value="{{ $partner->id }}"
                                            {{ old('parent_partner_id') == $partner->id ? 'selected' : '' }}>
                ...
                <div class="d-flex justify-content-end mt-4">
                    <a href="{{ route('admin.partner.retailers.list') }}" class="btn btn-light me-3">Cancel</a>
