<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('master.instrument-type') }}
    @endsection
    ...
    <div class="d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework">
        ...
        @if (isset($item))
            <div class="w-100 flex-lg-row-auto w-lg-400px mb-7 me-7 me-lg-10">
                <form class="form" method="POST"
                    action="{{ route('admin.master.instrument-type.update', ['instrument_type' => $item->id]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="card-body pt-0" data-select2-id="select2-data-128-idrh">
                            <div class="d-flex flex-column gap-10" data-select2-id="select2-data-127-fpwl">
                                ...
                                <div class="fv-row fv-plugins-icon-container">
                                    ...
                                    <div class="text-muted fs-7">Set the name of the instrument type.</div>
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('admin.master.instrument-type.index') }}" class="btn btn-light me-5">
                                    Cancel
                                </a>
        ...
        @else
            <div class="w-100 flex-lg-row-auto w-lg-400px mb-7 me-7 me-lg-10">
                <form class="form" method="POST" action="{{ route('admin.master.instrument-type.store') }}">
                    @csrf
                    ...
                    <div class="d-flex flex-column gap-5 gap-md-7">
                        <div class="table-responsive">
                            <table id="kt_datatable_dom_positioning"
                                ...
                                <tbody>
                                    @foreach ($instrumenttypeList as $item)
                                        <tr>
                                            <td>{{ ucfirst($item->name) }}</td>
                                            <td class="text-center">
                                                <a href="{{ route('admin.master.instrument-type.edit', ['instrument_type' => $item->id]) }}"
                                                    class="btn btn-primary hover-elevate-up btn-icon btn-sm me-1">
                                                    <i class="fas fa-pencil fs-6"></i>
                                                </a>
                                                <!-- Delete Form -->
                                                <form
                                                    action="{{ route('admin.master.instrument-type.destroy', ['instrument_type' => $item->id]) }}"
