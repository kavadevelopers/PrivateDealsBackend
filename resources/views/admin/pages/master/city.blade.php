<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('master.city') }}
    @endsection
    ...
    <div class="d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework">
        ...
        @if (isset($item))
            <div class="w-100 flex-lg-row-auto w-lg-400px mb-7 me-7 me-lg-10">
                <form class="form" method="POST"
                    action="{{ route('admin.master.city.update', ['city' => $item->id]) }}">
                    @csrf
                    @method('PUT')
                        ...
                        <div class="card-body pt-0" data-select2-id="select2-data-128-idrh">
                            <div class="d-flex flex-column gap-10 mb-5" data-select2-id="select2-data-127-fpwl">
                                ...
                                <div class="fv-row fv-plugins-icon-container">
                                    ...
                                    <div class="text-muted fs-7">Set the name of the city.</div>
                                ...
                                <div class="fv-row fv-plugins-icon-container">
                                    ...
                                    <select class="form-select" id="state_id" name="state_id" required
                                        ...
                                        @foreach ($state as $key => $value)
                                            <option value="{{ $value->id }}"
                                                {{ $item->state_id == $value->id ? 'selected' : '' }}>
                                                {{ $value->name }}
                                            </option>
                                        @endforeach
                                    </select>
                            ...
                            <div class="d-flex justify-content-end">
                                <a href="{{ route('admin.master.city.index') }}" class="btn btn-light me-5">
                                    Cancel
                                </a>
        ...
        @else
            <div class="w-100 flex-lg-row-auto w-lg-400px mb-7 me-7 me-lg-10">
                <form class="form" method="POST" action="{{ route('admin.master.city.store') }}">
                    @csrf
                            ...
                            <table id="kt_datatable_dom_positioning"
                                ...
                                <thead>
                                    <tr class="fw-semibold fs-6 text-gray-800">
                                        ...
                                        <th class="pe-7">State Name</th>
                                        {{-- <th class="pe-7">Country Name</th> --}}
                                ...
                                <tbody>
                                    @foreach ($citylist as $item)
                                        <tr>
                                            ...
                                            <td>{{ ucfirst($item->MasterState->name) }}</td>
                                            {{-- <td>{{ ucfirst($item->MasterCountry->name) }}</td> --}}
                                            <td class="text-center">
                                                <a href="{{ route('admin.master.city.edit', ['city' => $item->id]) }}"
                                                    class="btn btn-primary hover-elevate-up btn-icon btn-sm me-1">
                                                    <i class="fas fa-pencil fs-6"></i>
                                                </a>
                                                <!-- Delete Form -->
                                                <form
                                                    action="{{ route('admin.master.city.destroy', ['city' => $item->id]) }}"
