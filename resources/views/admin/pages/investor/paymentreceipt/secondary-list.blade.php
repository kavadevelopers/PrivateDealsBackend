<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('secondary-payment-receipt.list') }}
    @endsection
                            ...
                            <table id="kt_datatable_dom_positioning" class="table table-row-bordered gy-5 gs-7">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($list as $item)
                                        <tr>
                                            <td>{{ $item->transaction->buyer->name }}</td>
                                            <td class="text-center">
                                                ...
                                                @if ($item->status == App\Enums\Utills\StatusEnum::pending->value)
                                                    <a href="{{ route('admin.secpaymentReceipt.approve', ['id' => $item->id]) }}"
                                                        class="btn btn-success hover-elevate-up btn-icon btn-sm me-1"
                                                        title="Approve">
                                                        <i class="fas fa-check fs-6"></i>
                                                    </a>
                                                    <a href="{{ route('admin.secpaymentReceipt.reject', ['id' => $item->id]) }}"
