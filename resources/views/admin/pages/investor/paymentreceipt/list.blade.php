<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('primary-payment-receipt.list') }}
    @endsection
                            ...
                            <table id="kt_datatable_dom_positioning" class="table table-row-bordered gy-5 gs-7">
                                ...
                                <tbody>
                                    ...
                                    @foreach ($list as $item)
                                        <tr>
                                            <td>{{ $item->primaryTransaction->investor->name }}</td>
                                            <td>{{ $item->type }}</td>
                                            <td class="text-center">
                                                @if ($item->document)
                                                    <a href="{{ route('download.web', ['path' => $item->document->signed_path, 'name' => $item->document->meta->name]) }}"
                                                ...
                                                @if ($item->status == App\Enums\PaymentStatusEnum::pending->value)
                                                    <a href="{{ route('admin.paymentReceipt.approve', ['id' => $item->id]) }}"
                                                        class="btn btn-success hover-elevate-up btn-icon btn-sm me-1"
                                                        title="Approve">
                                                        <i class="fas fa-check fs-6"></i>
                                                    </a>
                                                    <a href="{{ route('admin.paymentReceipt.reject', ['id' => $item->id]) }}"
