    ...
    <div class="d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework">
        @if (isset($item))
            <div class="w-100 flex-lg-row-auto w-lg-1200px mb-7 me-7 me-lg-10">
                <form class="form" method="POST" action="{{ route('admin.investor.update', ['uuid' => $item->uuid]) }}"
                    ...
                    <div class="card card-flush py-4 mb-5">
                        ...
                        <div class="card-body pt-0">
                                    ...
                                    <select class="form-select" name="investor_type" aria-label="Select Investor Type">
                                        ...
                                        @foreach (App\Enums\InvestorTypeEnum::cases() as $type)
                                            <option value="{{ $type->value }}"
                                                {{ old('investor_type', $item->investor_type) === $type->value ? 'selected' : '' }}>
                                                {{ ucfirst($type->value) }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select" data-control="select2" data-placeholder="Select Country Code"
                                        ...
                                        @foreach ($supposted_countries as $supposted_country)
                                            <option value="{{ $supposted_country->code }}"
                                                {{ old('mobile_country_code', $item->mobile_country_code) == $supposted_country->code ? 'selected' : '' }}>
                            ...
                            @livewire('country-city-dropdown', [
                                'selectedCountry' => old('country_id', $item->country_id),
                                'selectedState' => old('state_id', $item->state_id),
                                'selectedCity' => old('city_id', $item->city_id),
                                'isOptional' => true,
                            ])
                            <div class="d-flex flex-wrap gap-10 mb-5">
                                <div class="fv-row w-100 flex-md-root">
                                    ...
                                    <input name="pincode" class="form-control mb-2 input" placeholder="Enter Pincode"
                                        tabindex="0" type="text" value="{{ old('pincode', $item->pincode) }}">
                                    @include('admin.partials.form.input-error-message', [
                                        'key' => 'pincode',
                                    ])
                                </div>
                                <div class="fv-row w-100 flex-md-root">
                                    ...
                                    <textarea name="address" class="form-control mb-2 input " placeholder="Enter Address" tabindex="0" type="text">{{ old('address', $item->address) }}</textarea>
                                    @include('admin.partials.form.input-error-message', [
                                        'key' => 'address',
                                    ])
                                </div>
                            ...
                            <div class="d-flex flex-wrap gap-10 mb-5">
                                <div class="fv-row w-100 flex-md-root">
                                    ...
                                    <select class="form-select" data-control="select2" data-placeholder="Select Partner" data-allow-clear="true"
                                        ...
                                        <option value=""></option>
                                        <option value="" {{ empty(old('partner_id', $item->partner_id)) ? 'selected' : '' }}>-- Select Partner --</option>
                                    ...
                                    <select class="form-select" data-control="select2"
                                        ...
                                        @foreach ($investors as $investor)
                                            <option value="{{ $investor->id }}"
                                                {{ old('parent_investor_id', $item->parent_investor_id) == $investor->id ? 'selected' : '' }}>
                                                {{ $investor->name }}
                                            </option>
                                        @endforeach
                                    </select>
                                    ...
                                    <select class="form-select" data-control="select2"
                                        ...
                                        @foreach ($relations as $relation)
                                            <option value="{{ $relation->id }}"
                                                {{ old('parent_investor_type', $item->family_relation_id) == $relation->id ? 'selected' : '' }}>
                                                {{ $relation->name }}
                                            </option>
                                        @endforeach
                                    </select>
