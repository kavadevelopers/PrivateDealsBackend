<x-default-layout>
    ...
    <div class="d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework">
        <div class="w-100 flex-lg-row-auto mb-7 me-7 me-lg-10">
            <form class="form" method="POST" action="{{ route('admin.company.promoterSave') }}"
                enctype="multipart/form-data">
                @csrf
                <div id="promoter-list" class="py-4">
                    @if ($company->promoters)
                        @foreach ($company->promoters as $item)
                            @include('admin.pages.company.promoter.row', [
                                'item' => $item,
                            ])
                        @endforeach
                    @else
                        @include('admin.pages.company.promoter.row', ['item' => false])
                    @endif
                </div>
                <div class="card card-flush py-4">
                    <div class="card-body">
                        <div class="d-flex flex-wrap gap-10  justify-content-end">
                            <button type="button" id="btn_add_promoter" class="btn btn-primary">
    ...
    @push('scripts')
        <script>
            ...
            $(document).ready(function() {
                $('#btn_add_promoter').click(function() {
                    $('#promoter-list').append(`{!! addslashes(view('admin.pages.company.promoter.row', ['item' => false])->render()) !!}`);
                });
