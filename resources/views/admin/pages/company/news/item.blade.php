<x-default-layout>
    ...
    <div class="d-flex flex-column flex-lg-row fv-plugins-bootstrap5 fv-plugins-framework">
        <div class="w-100 flex-lg-row-auto mb-7 me-7 me-lg-10">
            <form class="form" method="POST" action="{{ route('admin.company.newsSave') }}"
                                            ...
                                            @foreach ($company->news as $key => $news)
                                                <tr>
                                                    <td class="text-center">
                                                        ...
                                                        @if ($news->image && $news->image != null && $news->image != '')
                                                            <p><a
                                                                    href="{{ route('download.web', ['path' => $news->image, 'name' => 'File of ' . $company->brand_name . ' - Event ' . $news->title]) }}">Download</a>
                                                            </p>
                                                        @endif
                                                    ...
                                                    <td><input type="text" name="input[{{ $key }}][title]"
                                                            ...
                                                            value="{{ $news->title }}" required></td>
                                                    <td>
                                                        <textarea name="input[{{ $key }}][description]" placeholder="Description" class="form-control" required>{{ $news->description }}</textarea>
                                                    </td>
                                                    <td><input type="text" name="input[{{ $key }}][link]"
                                                            placeholder="Link" class="form-control"
                                                            value="{{ $news->link }}" required></td>
                                                    <td class="text-center">
                                                        <button
            ...
            $(document).ready(function() {
                ...
                $('#btn-add-mem').on('click', function() {
                 ...
                 <td class="text-center">
                    ...
                    <input type="file" class="form-control" name="input[` + i + `][file]" onchange="fileExAllowedWithSize(this,'.png,.jpg,.jpeg','{{ CommonHelper::appSettings('file_image_max_size') }}')" required>
                 </td>
                 ...
                 <td><input type="text" name="input[` + i + `][link]" placeholder="Link" class="form-control" required></td>
                 <td class="text-center">
