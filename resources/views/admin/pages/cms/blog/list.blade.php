            ...
            <div class="card card-flush py-4">
                ...
                <div class="card-header">
                    ...
                    <div class="card-toolbar">
                        <a href="{{ route('admin.cms.blog.create') }}" class="btn btn-sm btn-primary">
                            Create
                        </a>
                    </div>
                </div>
                                            ...
                                            <td>
                                                <div class="symbol symbol-50px symbol-2by3">
                                                    <img class="shimmer lazy"
                                                        data-src="{{ FileUpDownHelper::get_page_blog_banner_url($item->banner) }}" />
                                                </div>
                                            </td>
                                            <td>{{ ucfirst($item->type == 1 ? 'Blog' : 'Third-party Link') }}
                                            </td>
                                            <td>{{ ucfirst($item->title) }}</td>
                                            <td>{{ ucfirst($item->display_order) }}</td>
                                            <td class="text-center">
                                                <a href="{{ route('admin.cms.blog.edit', ['blog' => $item->id]) }}"
                                                    class="btn btn-primary hover-elevate-up btn-icon btn-sm me-1">
                                                    <i class="fas fa-pencil fs-6"></i>
                                                </a>
                                                <!-- Delete Form -->
                                                <form
                                                    action="{{ route('admin.cms.blog.destroy', ['blog' => $item->id]) }}"
