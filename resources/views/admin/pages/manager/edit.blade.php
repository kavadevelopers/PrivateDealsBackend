<x-default-layout>
    ...
    @section('breadcrumbs')
        {{ Breadcrumbs::render('manager.edit') }}
    @endsection

    <div class="d-flex flex-column flex-lg-row">
        @if (isset($item))
            <div class="w-100 flex-lg-row-auto w-lg-1200px mb-7 me-7 me-lg-10">
                <form class="form" method="POST" action="{{ route('admin.manager.update', ['uuid' => $item->uuid]) }}"
                            ...
                            <div class="d-flex flex-wrap gap-10">
                                    ...
                                    <select class="form-select" name="role" aria-label="Select example">
                                        ...
                                        @foreach (App\Enums\AdminTypeEnum::cases() as $adminType)
                                            <option value="{{ $adminType }}"
                                                {{ old('role', $item->role) == $adminType->value ? 'selected' : '' }}>
                                                {{ $adminType }}
                                            </option>
                                        @endforeach
                                    </select>
                                ...
                                <div class="fv-row w-100 flex-md-root">
                                    ...
                                    <input name="username" class="form-control mb-2 input" placeholder="Enter Username"
                                        tabindex="0" type="text" value="{{ old('username', $item->username) }}">
                                    @include('admin.partials.form.input-error-message', [
                                        'key' => 'username',
                                    ])
                                </div>
                                    ...
                                    @foreach ($permissions as $permission)
                                        <label
                                            ...
                                            <input class="form-check-input" type="checkbox" name="permissions[]"
                                                ...
                                                {{ in_array($permission->name, old('permissions', $item->permissions->pluck('name')->toArray())) ? 'checked' : '' }}>
